...
function Header() {
	...
	return (
		<header className='backdrop-blur-2xl flex flex-wrap sm:justify-start sm:flex-nowrap w-full shadow-md fixed top-0  z-50'>
			<nav
				className='max-w-[85rem] w-full mx-auto px-6 flex flex-wrap basis-full items-center justify-between'
				aria-label='Global'
			>
				<Link href={'/'}>
					{theme === 'light' ? (
						...
						<Image
							src={logo2}
							alt='logo'
							className=' sm:ml-10 relative lg:right-10'
							height={'45'}
						/>
					)}
				</Link>
				...
				>
					<div className='flex flex-col gap-12 mt-5 sm:flex-row sm:items-center sm:mt-0 sm:ps-5 p-7'>
						<Link
							className='font-medium text-slate-950 hover:text-gray-400 dark:text-slate-50 dark:hover:text-gray-500'
							href='/about'
						>
							{t('link1')}
						</Link>
